...
public class BinaryWallpaperService extends WallpaperService {

	@Override
	public Engine onCreateEngine() {
		return new BinaryWallpaperEngine();
	}

	private class BinaryWallpaperEngine extends Engine {
		...
		private void draw() {
			...
			try {
				canvas = holder.lockCanvas();
				if( canvas!=null ){
					if( circles.size()>=maxNumber ) circles.clear();
		...
		private void drawCircles(Canvas canvas, List<MyPoint> circles) {
			canvas.drawColor(Color.BLACK);
			for(MyPoint point : circles) canvas.drawCircle(point.x, point.y, 20.0f, paint);
